...
namespace dms {
  ...
  float Option::get_float(string k){
    ...
    if (key_type.find(k) == key_type.end())
      {
	stringstream ss1;
	cerr << "unused options: \"" << k << "\", please check the program source";
	exit(1);
	return -1;
      }
    stringstream ss2;
    ...
    if (ss2.fail() or ss2.bad() or !ss2.eof())
      {
	stringstream ss3("");
	cerr << "invalid float value for option: \"" << k.c_str() << "\", please check your input options";
	exit(1);
	return -1;
      }
    return ret;
  ...
  int Option::get_int(string k){
    ...
    if (ss2.fail() or ss2.bad() or !ss2.eof())
      {
	stringstream ss3("");
	cerr << "invalid int value for option: \"" << k.c_str() << "\", please check your input options";
	exit(1);
	return -1;
      }
    return ret;
  ...
  void Option::dump_help(){
    ...
    for (map< string, string >::iterator iter = key_comment.begin(); iter != key_comment.end(); iter ++)
      {
	if ( key_musthave.find(iter->first)==key_musthave.end() ) // not a "must have" option
	  {
	    cout << '\t' << iter->first << " : " << iter->second << endl;
  ...
  Option::PARSE_RETURN_TYPE Option::parse(int argc, char** argv){
	...
	if (key_type.find(s)!=key_type.end()) // if it is registered
	  ...
	  {
	    cerr << "unkown options: \""<<argv[i]<<"\", use --help to see more info" << endl;
	    return PARSE_ERROR;
	  }
      }
    if (!kmh.empty())
	...
	for ( set < string >::iterator iter = kmh.begin(); iter != kmh.end(); iter ++)
	  {
	    cerr << "option \""<<(*iter)<<"\" must be used" << endl;
	  }
	return PARSE_ERROR;
      }
